...
// All routes not matched below fall through to the SPA fallback
...
app.post("/badge", function(req, res) {
  ...
  badges.set(id, { score: b.score, grade: b.grade, credits: b.credits,
...
app.get("/badge/:id", function(req, res) {
  ...
  if (!d) return res.status(404).send("<h2 style='font-family:monospace;color:#e05c5c;background:#060e06;padding:40px'>Badge not found or session expired.</h2>");
  ...
  res.send("<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><title>GSCS Badge</title>"
  ...
  +".t{font-size:18px;font-weight:700;color:#00e887}.s{font-size:11px;color:#4a7a4a;margin-top:3px;letter-spacing:1px}"
  ...
  +"<div class='h'><div style='font-size:32px'>&#127807;</div><div><div class='t'>Green Software Certified</div><div class='s'>GREEN SOFTWARE CREDIT SYSTEM</div></div></div>"
...
app.post("/certificate", function(req, res) {
  ...
  var id = Math.random().toString(36).slice(2, 10);
  certificates.set(id, { projectName: b.projectName||"Unnamed", summary: b.summary,
    top_issues: b.top_issues||[], files: b.files||[], created_at: new Date().toISOString() });
  res.json({ id: id, cert_url: "/certificate/" + id });
});

app.get("/certificate/:id", function(req, res) {
  var d = certificates.get(req.params.id);
  if (!d) return res.status(404).send("<h2 style='font-family:monospace;color:#e05c5c;background:#060e06;padding:40px'>Certificate not found or session expired.</h2>");
  var cc  = {Platinum:"#e2e8f0",Gold:"#f5c542",Silver:"#b0b8c1",Bronze:"#cd7f32",None:"#e05c5c"};
  var gc  = {A:"#00e887",B:"#7bd4a0",C:"#f5c542",D:"#e05c5c"};
  ...
  res.setHeader("Content-Type","text/html");
  res.send("<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><title>GSCS Certificate - "+d.projectName+"</title>"
  ...
  +"<div class='is'>Certificate issued "+iss+"</div></div>"
  +"<div class='stats'>"
  +"<div class='stat'><div class='sv'>"+d.summary.overall_score+"</div><div class='sl'>OVERALL SCORE</div></div>"
...
app.get("/health", function(_req, res) {
  res.json({ status: "ok", runtime: "browserpod", hasApp: require("fs").existsSync(require("path").join(__dirname,"static","index.html")) });
});

// SPA fallback — React Router handles /audit /scan /dashboard etc
app.get("*", function(_req, res) {
  var idx = require("path").join(__dirname, "static", "index.html");
  require("fs").existsSync(idx)
    ? res.sendFile(idx)
    : res.status(200).send("<h2 style='font-family:monospace;color:#00e887;background:#060e06;padding:40px'>GSCS Pod server running &mdash; run <code>npm run build</code> to deploy the full app here.</h2>");
});
